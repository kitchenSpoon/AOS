...
static int _load_segment(pid_t pid, addrspace_t *as,
    ...
    dprintf(3, "load segment into vspace\n");
...
static void _load_segment2(void *token, int err){
    dprintf(3, "load segment into vspace2\n");
    ...
    if(cont->pos < cont->file_size){
        ...
        //dprintf(3, "load segment into vspace2 page map\n");
        if (!sos_page_is_inuse(cont->as, vpage)) {
            ...
            dprintf(3, "_load_segment2: wanna_read = %lu\n", cont->wanna_read);
    ...
    //dprintf(3, "load segment into vspace2 out\n");
...
_load_segment3(uintptr_t token, enum nfs_stat status,
    ...
    dprintf(3, "_load_segment3 called\n");
    load_segment_cont_t *cont = (load_segment_cont_t*)token;
...
void elf_load(pid_t pid, addrspace_t *as, char *file_name,
    ...
    if (status != RPC_OK) {
        dprintf(3, "elf_load nfs_lookup err\n");
...
_elf_load_lookup_cb(uintptr_t token, enum nfs_stat status, fhandle_t *fh, fattr_t *fattr) {
    dprintf(3, "_elf_load_lookup_cb called\n");
    elf_load_cont_t *cont = (elf_load_cont_t*)token;

    if (status != NFS_OK) {
        dprintf(3, "_elf_load_lookup_cb: nfs_lookup status != NFS_OK\n");
        _elf_load_end((void*)cont, EFAULT);
        return;
    }
    ...
    if(cont->fh == NULL){
        dprintf(3, "_elf_load_lookup_cb: no mem for fh\n");
    ...
    if (rpc_status != RPC_OK) {
        dprintf(3, "_elf_load_lookup_cb: nfs_read failed\n");
        _elf_load_end((void*)cont, EFAULT);
    }
}
...
_elf_load_read_elfheader_cb(uintptr_t token, enum nfs_stat status,
                              fattr_t *fattr, int count, void *data) {
    dprintf(3, "_elf_load_read_elfheader_cb called\n");
    elf_load_cont_t *cont = (elf_load_cont_t*)token;

    if (status != NFS_OK || count != sizeof(struct Elf32_Header)) {
        dprintf(3, "_elf_load_read_elfheader_cb: err\n");
        _elf_load_end((void*)cont, EFAULT);
        return;
    }
    ...
    if (elf_checkFile(elf_hdr)) {
        dprintf(3, "_elf_load_read_elfheader_cb: elf_check failed\n");
    ...
    if (rpc_status != RPC_OK) {
        dprintf(3, "_elf_load_read_elfheader_cb: nfs_read failed\n");
        _elf_load_end((void*)cont, EFAULT);
    }
}
...
_elf_load_read_progheaders_cb(uintptr_t token, enum nfs_stat status,
                              fattr_t *fattr, int count, void *data) {
    dprintf(3, "_elf_load_read_progheaders_cb called\n");
    elf_load_cont_t *cont = (elf_load_cont_t*)token;

    if (status != NFS_OK || count != cont->ph_size * cont->ph_num) {
        dprintf(3, "_elf_load_read_progheaders_cb: err\n");
        _elf_load_end((void*)cont, EFAULT);
        return;
    }
    ...
    if (cont->prog_hdrs == NULL) {
        dprintf(3, "_elf_load_read_progheaders_cb: no memory for prog_hdrs\n");
        _elf_load_end((void*)cont, ENOMEM);
        return;
    }
...
_elf_load_load_segments(void* token, int err){
    ...
    if(cont->i < cont->ph_num) {
        dprintf(3, "cont->i = %d, cont->ph_num = %d\n", cont->i,cont->ph_num);
...
_elf_load_end(void *token, int err) {
    dprintf(3, "_elf_load_end called\n");
    elf_load_cont_t *cont = (elf_load_cont_t*)token;
