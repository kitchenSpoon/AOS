...
static void load_segment2(void* token, int err){
    ...
    if(cont->pos < cont->file_size){
        ...
        if (!sos_page_is_inuse(cont->as, vpage)) {
            ...
            sos_page_map(cont->pid, cont->as, vpage, cont->permissions, load_segment2, token, false);
        } else {
            seL4_Word kdst;
            err = sos_get_kvaddr(cont->as, cont->dst, &kdst);
            if (err) {
...
static int load_segment(pid_t pid, addrspace_t *as,
                                    unsigned long file_offset,
                                    unsigned long segment_size,
...
elf_load_part2(void* token, int err){
    ...
    if(cont->i < cont->ph_num) {
        ...
        err = load_segment(cont->pid, cont->as, file_offset, segment_size, file_size,
...
void elf_load(pid_t pid, addrspace_t* as, char *file_name, process_t* proc, elf_load_cb_t callback, void* token) {
