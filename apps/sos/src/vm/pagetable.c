...
void sos_page_map_part4(void* token){
    ...
    int err = frame_alloc(sos_page_map_part5, token, cont->noswap);
    if (err) {
        cont->callback(cont->token, EINVAL);
        free(cont);
        return;
...
sos_page_map_part2(void* token, seL4_Word kvaddr){
    ...
    int err = frame_alloc(sos_page_map_part3, token, true);
    if (err) {
        frame_free(kvaddr);
...
sos_page_map(addrspace_t *as, seL4_Word vaddr, uint32_t permissions,
    ...
    if (as->as_pd_regs[x] == NULL) {
        ...
        err = frame_alloc(sos_page_map_part2, (void*)cont, true);
        if (err) {
            free(cont);
            return EFAULT;
        }
